...
// Note by Abhi Nayar, 05/05/2018
// Used under Creative Commons license from https://github.com/askmike/realtime-webgl-globe
 ...
 */
var Globe = function Globe(container) {
  var PI = Math.PI;
  var PI_HALF = PI / 2;
  ...
  var createMesh = {
    earth: function() {
      ...
      var material = new THREE.ShaderMaterial({
        uniforms: {
          texture: { type: 't', value: THREE.ImageUtils.loadTexture('https://abhinayar.github.io/BirthOfARevolution/src/public/img/colored_md.jpg')}
        },
        vertexShader: [
        ...
        fragmentShader: [
          ...
          'void main() {',
            ...
            //'vec3 atmosphere = vec3( 0.6, 0.5, 0.0 ) * pow( intensity, 5.0 );',
            'vec3 atmosphere = vec3( 0.625, 0.55, 0.425 ) * pow( intensity, 2.5 );',
    ...
    atmosphere: function() {
      var material = new THREE.ShaderMaterial({
        ...
        fragmentShader: [
          'varying vec3 vNormal;',
          'void main() {',
            'float intensity = pow( 0.72 - dot( vNormal, vec3( 0, 0, 1.0 ) ), 12.0 );',
            //'gl_FragColor = vec4( 1, 0.8, 0.0, 1.0 ) * intensity;',
            'gl_FragColor = vec4( 1.25, 1.1, 0.95, 1.0 ) * intensity;',
          '}'
        ].join('\n'),
        side: THREE.BackSide,
